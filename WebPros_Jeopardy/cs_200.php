<?php
    session_start();
    include '_dbconnect.php'
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Computer Science</title>
    <link rel="stylesheet" href="questions_answers.css">
</head>
<body>
    <div class>
        <fieldset>
        <legend><h1 class="category">Computer Science</h1></legend>
        <br><br>
        <a href="leaderboard.php"><input type="submit" value="See Leaderboard"></a><br>
        <a href="logout.php"><input type="submit" value="Logout"></a><br>
        <a href="jeopardy_board.php"> <input type="submit" value="Back to game board"></a>
        <p>This notation is used to determine the run-time and space usage of a program</p><br><br>
        <form  action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
            <label for="csQ2">Choose your answer:</label><br><br>
            <input type="submit" name="csq2" id="csQ2" value="What is Big-O?">
            <input type="submit" name="csq2" id="csQ2" value="What is Little-O?">
            <input type="submit" name="csq2" id="csQ2" value="What is Big-Omega?">
            <input type="submit" name="csq2" id="csQ2" value="What is Little-Omega">
            <br><br>
        </form>

        <?php
            if(isset($_POST['csq2'])){
                $answer = $_POST['csq2'];
                if((strcmp($answer, "What is Big-O?"))==0){
                    $sql = "UPDATE `demo-registration`.`user_points` SET score = score + 200";
                    mysqli_query($conn, $sql);
                    echo "<div class='right-wrong'>";
                    echo "<p class='correct'>Correct</p>";
                    echo "</div>";
                }else{
                    echo "<div class='right-wrong'>";
                    echo "<p class='wrong'>Incorrect</p>";
                    echo "</div>";
                }
            }?>
        </fieldset>
    </div>


</body>
</html>